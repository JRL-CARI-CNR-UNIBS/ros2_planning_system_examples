...
ActionCostPtr MoveActionCostBase::compute_action_cost(const geometry_msgs::msg::PoseStamped & goal)
{
    std::cerr << "Compute action cost" << std::endl;
    action_cost_ptr_ = std::make_shared<plansys2_msgs::msg::ActionCost>();
    ...
    send_goal_options.result_callback = [this](ComputePathResult & result)
    {
        ...
        if (result.code != rclcpp_action::ResultCode::SUCCEEDED) {
            ...
            this->action_cost_ptr_->nominal_cost = std::numeric_limits<double>::infinity();
            this->action_cost_ptr_->std_dev_cost = 0.0;
        ...
        action_cost_ptr_ = this->compute_cost_function();

        RCLCPP_DEBUG(
            node_->get_logger(), "Computed path cost: %f", this->action_cost_ptr_->nominal_cost);
        std::cerr << "Computed path cost: " << this->action_cost_ptr_->nominal_cost << std::endl;
        // action_executor_client_->set_action_cost(action_cost);
        // action_executor_client_->send_response(msg);
    ...
    send_goal_options.goal_response_callback =
        [this](const ComputePathGoalHandle::SharedPtr & goal_handle)
        {
            if (!goal_handle) {
    ...
    auto future_result = compute_path_action_client_->async_send_goal(path_to_pose_goal); // , send_goal_options);
